...
class VolatilityGridTests(unittest.TestCase):
    def test_config_rejects_vol_period_one(self):
        with self.assertRaisesRegex(ValueError, "vol precisa ser >= 2"):
    ...
    def test_vol_period_two_can_pass_positive_vol_gate(self):
        closes = np.array([10.0, 11.0, 9.0, 12.0, 12.5], dtype=np.float64)
        gate = opt.sample_vol_positive(closes, 2)
        self.assertEqual(gate.dtype, np.bool_)
        self.assertTrue(bool(np.any(gate)))
