...
class ReferenceEngineSyntheticTests(unittest.TestCase):
    ...
    def test_indicators_targets_and_portfolio_match(self):
        ...
        for g, s, m in [(5, 2, 5), (20, 9, 30), (40, 20, 63), (80, 60, 252)]:
            ...
            self.assertAlmostEqual(float(fast["final_equity"][0]), float(slow["final_equity"]), places=7)
            self.assertEqual(int(fast["holding"][0]), int(slow["holding"]))
            self.assertEqual(int(fast["shares"][0]), int(slow["shares"]))
